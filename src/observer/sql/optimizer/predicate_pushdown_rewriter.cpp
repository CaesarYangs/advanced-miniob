...
RC PredicatePushdownRewriter::rewrite(std::unique_ptr<LogicalOperator> &oper, bool &change_made)
{
  ...
  for(auto &child: oper->children()){
    ...
    predicate_pushdown_rewriter(child,predicate_expr, change_made);
  }
  if (!predicate_expr) {
    // 所有的表达式都下推到了下层算子
...
RC PredicatePushdownRewriter::predicate_pushdown_rewriter
                  (std::unique_ptr<LogicalOperator> &oper,std::unique_ptr<Expression> &predicate_expr,bool &change_made){
  RC rc = RC::SUCCESS;
  for(auto &child: oper->children()){
    std::vector<std::unique_ptr<Expression>> pushdown_exprs;
    if(child->type() == LogicalOperatorType::TABLE_GET){
    ...
    std::unique_ptr<Expression> &expr, std::vector<std::unique_ptr<Expression>> &pushdown_exprs,TableGetLogicalOperator* table_get_oper)
{
  ...
  if (expr->type() == ExprType::CONJUNCTION) {
    ...
    if (comp != EQUAL_TO && comp != EQUAL_TO && comp !=  LESS_EQUAL && comp != NOT_EQUAL 
    && comp != LESS_THAN && comp != GREAT_EQUAL && comp != GREAT_THAN) {   //省去了Like比较
    ...
    ExprType right_expr_type = right_expr->type();
    ExprType left_expr_type = left_expr->type();
    ...
    const char *right_table_name = table_get_oper->table()->name();
    if(left_field_expr->table_name() == table_get_oper->table()->name()){
